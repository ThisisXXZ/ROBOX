...
from robox.robox import get_robox_path
...
import robox.robox.utils.download_utils as download_utils
from robox.robox import get_robox_path
...
DATASET_LINKS = {
    "robox_object": "https://utexas.box.com/shared/static/avkklgeq0e1dgzxz52x488whpu8mgspk.zip",
    "robox_goal": "https://utexas.box.com/shared/static/iv5e4dos8yy2b212pkzkpxu9wbdgjfeg.zip",
    "robox_spatial": "https://utexas.box.com/shared/static/04k94hyizn4huhbv5sz4ev9p2h1p6s7f.zip",
    "robox_100": "https://utexas.box.com/shared/static/cv73j8zschq8auh9npzt876fdc1akvmk.zip",
}

HF_REPO_ID = "yifengzhu-hf/ROBOX-datasets"


def download_from_huggingface(dataset_name, download_dir, check_overwrite=True):
    """
    Download a specific ROBOX dataset from Hugging Face.
    
    Args:
        dataset_name (str): Name of the dataset to download (e.g., 'robox_spatial')
...
def robox_dataset_download(datasets="all", download_dir=None, check_overwrite=True, use_huggingface=False):
    """Download robox datasets
    ...
    if download_dir is None:
        download_dir = get_robox_path("datasets")
...
def check_robox_dataset(download_dir=None):
    ...
    for dataset_name in [
        ...
        if os.path.exists(dataset_dir):
            ...
            for path in Path(dataset_dir).glob("*.hdf5"):
                count += 1
            if (count == 10 and dataset_name != "robox_90") or (
                count == 90 and dataset_name == "robox_90"
            ):
                dataset_status = True
                info_str = colored(
