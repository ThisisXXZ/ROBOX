...
def postprocess_model_xml(xml_str, cameras_dict={}, demo_generation=False):
    ...
    # also replace paths for robox
    robox_path = os.getcwd() + "/robox"
    robox_path_split = robox_path.split("/")
    ...
    for elem in all_elements:
        ...
        elif "robox" in old_path_split and demo_generation:
            ind = max(
                loc for loc, val in enumerate(old_path_split) if val == "robox"
            )  # last occurrence index
            new_path_split = robox_path_split + old_path_split[ind + 1 :]
            new_path = "/".join(new_path_split)
            elem.set("file", new_path)
        else:
            continue
