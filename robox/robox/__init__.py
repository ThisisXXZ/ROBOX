...
robox_config_path = os.environ.get(
    "ROBOX_CONFIG_PATH", os.path.expanduser("~/.robox")
)
config_file = os.path.join(robox_config_path, "config.yaml")
...
def set_robox_default_path(custom_location=os.path.dirname(os.path.abspath(__file__))):
    print(
        f"[Warning] You are changing the default path for Robox config. This will affect all the paths in the config file."
    )
...
if not os.path.exists(robox_config_path):
    os.makedirs(robox_config_path)
