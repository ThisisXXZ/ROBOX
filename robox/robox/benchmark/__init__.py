...
from robox.robox.benchmark.robox_suite_task_map import robox_task_map
...
task_maps = {}
max_len = 0
for robox_suite in robox_suites:
    task_maps[robox_suite] = {}

    for task in robox_task_map[robox_suite]:
        language = grab_language_from_filename(task + ".bddl")
        task_maps[robox_suite][task] = Task(
            name=task,
            language=language,
            problem="Robox",
            problem_folder=robox_suite,
...
@register_benchmark
class ROBOX_SPATIAL(Benchmark):
    def __init__(self, task_order_index=0):
        super().__init__(task_order_index=task_order_index)
        self.name = "robox_spatial"
        self._make_benchmark()


@register_benchmark
class ROBOX_OBJECT(Benchmark):
    def __init__(self, task_order_index=0):
        super().__init__(task_order_index=task_order_index)
        self.name = "robox_object"
        self._make_benchmark()


@register_benchmark
class ROBOX_GOAL(Benchmark):
    def __init__(self, task_order_index=0):
        super().__init__(task_order_index=task_order_index)
        self.name = "robox_goal"
        self._make_benchmark()


@register_benchmark
class ROBOX_90(Benchmark):
    def __init__(self, task_order_index=0):
        super().__init__(task_order_index=task_order_index)
        assert (
            task_order_index == 0
