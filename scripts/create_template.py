"""
This is a script for creating various files frrom templates. This is to ease the process for users who want to extend ROBOX, creating new tasks. You would still need to make necessary changes based on the template to serve your own need, but the hope is that we save you much time by providing the necessar templates.
"""
...
from robox.robox.envs.textures import get_texture_file_list


def create_problem_class_from_file(class_name):
    template_source_file = os.path.join(
        get_robox_path("benchmark_root"), "../../templates/problem_class_template.py"
    )
...
def create_scene_xml_file(scene_name):
    ...
    template_source_file = os.path.join(
        get_robox_path("benchmark_root"), "../../templates/scene_template.xml"
    )
    ...
    print(
        "\n [Notice] The texture fiile paths are specified in the relative path format assuming your scene xml will be placed in the path robox/robox/assets/scenes/. "
    )
    return
